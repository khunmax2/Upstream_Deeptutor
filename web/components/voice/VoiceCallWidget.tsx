...
import {
  VOICE_ACTION_EVENT,
  type VoiceActionDetail,
} from "./VoiceActionBridge";
import {
  attachPageAgentBridge,
  type PageAgentBridge,
} from "@/lib/page-actuator/wsBridge";
...
const UI_PAGES: { id: string; label: string; path: string }[] = [
  ...
  {
    id: "partners",
    label: "หน้า partners (เพื่อนคู่คิด/บอทคู่หู)",
    path: "/partners",
  },
  {
    id: "playground",
...
function initMascot(THREE: any, canvas: HTMLCanvasElement): MascotHandle {
  ...
  [-1, 1].forEach((s) => {
    figure.add(
      mesh(new THREE.CylinderGeometry(0.17, 0.15, 0.34, 24), mat.suit, [
        s * 0.18,
        0.32,
        0,
      ]),
    ...
    figure.add(
      mesh(new THREE.BoxGeometry(0.02, 0.3, 0.02), mat.suitDk, [
        s * 0.18,
        0.32,
        0.15,
      ]),
    ...
    const foot = mesh(
      new THREE.SphereGeometry(0.155, 30, 22),
      mat.gold,
      [0, 0.12, 0.12],
    );
    ...
    sh.add(heel);
    sh.add(
      mesh(new THREE.SphereGeometry(0.018, 10, 8), mat.gold, [0, 0.2, 0.16]),
    );
    figure.add(sh);
  });
  ...
  figure.add(torso);
  const bodyMesh = mesh(
    new THREE.SphereGeometry(0.42, 40, 30),
    mat.suit,
    [0, 0.78, 0],
  );
  ...
  torso.add(
    mesh(
      new THREE.CylinderGeometry(0.17, 0.19, 0.16, 22),
      mat.white,
      [0, 1.16, 0.02],
    ),
  );
  torso.add(
    mesh(
      new THREE.TorusGeometry(0.21, 0.07, 14, 30),
      mat.suit,
      [0, 1.11, 0],
      [Math.PI / 2, 0, 0],
    ),
  ...
  torso.add(
    mesh(new THREE.BoxGeometry(0.085, 0.085, 0.05), mat.tie, [0, 1.0, 0.42]),
  );
  const blade = mesh(
    new THREE.CylinderGeometry(0.05, 0.085, 0.33, 4),
    mat.tie,
    [0, 0.79, 0.41],
  );
  ...
  torso.add(blade);
  torso.add(
    mesh(new THREE.SphereGeometry(0.022, 12, 10), mat.dark, [0, 0.55, 0.37]),
  );
  torso.add(
    mesh(
  ...
  torso.add(
    mesh(new THREE.ConeGeometry(0.035, 0.06, 4), mat.gold, [0.17, 0.92, 0.4]),
  );
  ...
  function arm(side: number) {
    ...
    const cap = mesh(
      new THREE.SphereGeometry(0.17, 26, 20),
      mat.suit,
      [0, 0, 0],
    );
    ...
    g.add(
      mesh(
        new THREE.CylinderGeometry(0.15, 0.115, 0.42, 22),
        mat.suit,
        [0, -0.23, 0],
      ),
    );
    g.add(
      mesh(
        new THREE.CylinderGeometry(0.13, 0.125, 0.05, 22),
        mat.suit,
        [0, -0.44, 0],
      ),
    );
    const glove = mesh(
      new THREE.SphereGeometry(0.155, 26, 20),
      mat.blue,
      [0, -0.51, 0.02],
    );
    ...
    g.add(glove);
    g.add(
      mesh(new THREE.SphereGeometry(0.06, 16, 12), mat.blue, [
        side * -0.1,
        -0.47,
        0.09,
      ]),
  ...
  const facePanel = mesh(
    new THREE.SphereGeometry(0.45, 42, 32),
    mat.cream,
    [0, -0.02, 0.33],
  );
  ...
  function eye(side: number) {
    ...
    g.add(e);
    g.add(
      mesh(new THREE.SphereGeometry(0.032, 14, 12), mat.hi, [
        side * 0.04,
        0.07,
        0.09,
      ]),
    ...
    g.add(
      mesh(new THREE.SphereGeometry(0.016, 12, 10), mat.hi, [
        -side * 0.03,
        -0.03,
        0.1,
      ]),
  ...
  const eyeR = eye(1);
  [-1, 1].forEach((s) => {
    const ch = mesh(new THREE.SphereGeometry(0.075, 18, 14), mat.cheek, [
      s * 0.27,
      -0.06,
      0.46,
    ]);
  ...
  const mouth = mesh(
    new THREE.SphereGeometry(0.08, 28, 22),
    mat.mouth,
    [0, -0.23, 0.52],
  );
  head.add(mouth);
  ...
  [-1, 1].forEach((s) =>
    head.add(
      mesh(new THREE.SphereGeometry(0.085, 18, 14), mat.blue, [
        s * 0.43,
        0.28,
        0.3,
      ]),
  ...
  function resize() {
    const w =
      canvas.clientWidth || canvas.parentElement?.clientWidth || innerWidth;
    const h =
      canvas.clientHeight || canvas.parentElement?.clientHeight || innerHeight;
    camera.aspect = w / h;
...
export default function VoiceCallWidget() {
  ...
  const reportActionResult = useCallback(
    (
      target: string,
      field: string,
      verdict: UiActionVerdict,
      argument = "",
    ) => {
      if (!verdict.ok) {
        addMsg(
          "sys",
          `⚠ ตรวจผลแล้วยังไม่สำเร็จ: ${target}${field ? ` (${field})` : ""}`,
        );
      }
  ...
  const executeUiAction = useCallback(
    (m: any) => {
      ...
      if (page) {
        ...
        void verifyPath(page.path).then((v) =>
          reportActionResult(target, "", v),
        );
        return;
      }
      switch (target) {
        case "open_path": {
          ...
          if (!allowed || !argument.startsWith("/")) {
            ...
            reportActionResult(
              target,
              "",
              { ok: false, detail: "path_not_allowed" },
              argument,
            );
            return;
          }
          ...
          void verifyPath(argument).then((v) =>
            reportActionResult(target, "", v, argument),
          );
          return;
        }
        case "click_index": {
          ...
          void (async () => {
            ...
            if (!el) {
              ...
              reportActionResult(
                target,
                "",
                { ok: false, detail: "scan_target_gone" },
                argument,
              );
              return;
            }
        ...
        case "click_element": {
          ...
          void (async () => {
            const el = argument
              ? await findWithPoll(() =>
                  findClickableByText(argument, panelRef.current),
                )
              : null;
            if (!el) {
              ...
              reportActionResult(
                target,
                "",
                { ok: false, detail: "element_not_found" },
                argument,
              );
              return;
            }
        ...
        case "fill_field": {
          ...
          void (async () => {
            const fieldEl =
              field && argument
                ? await findWithPoll(() =>
                    findFieldElement(field, panelRef.current),
                  )
                : null;
            if (fieldEl) {
              ...
              if (written === null) {
                reportActionResult(target, field, {
                  ok: false,
                  detail: "set_failed",
                });
                return;
              }
              let verdict = await verifyFieldValue(
                field,
                written,
                panelRef.current,
              );
          ...
          void (async () => {
            ...
            if (focusEl) {
              ...
              reportActionResult(
                target,
                field,
                await verifyFieldFocused(field, panelRef.current),
              );
            } else {
              addMsg("sys", `⚠ หาช่อง "${field}" บนจอไม่เจอแล้ว`);
              reportActionResult(target, field, {
        ...
        case "edit_field": {
          ...
          if (editEl) {
            void (async () => {
              ...
              const expected = editFieldByVoice(
                field,
                argument,
                panelRef.current,
              );
        ...
        case "open_kb":
          ...
          router.push(
            argument
              ? `/knowledge?kb=${encodeURIComponent(argument)}`
              : "/knowledge",
          );
          void verifyPath("/knowledge").then((v) =>
            reportActionResult(target, "", v),
          );
          return;
        case "type_in_chat": {
          ...
          window.dispatchEvent(
            new CustomEvent(VOICE_ACTION_EVENT, { detail: typeDetail }),
          );
          if (typed) {
    ...
    <>
      ...
      {mounted && (
        ...
        >
          ...
          <div
            style={{ position: "relative", height: 240, pointerEvents: "none" }}
          >
            <canvas
              ref={canvasRef}
              style={{
