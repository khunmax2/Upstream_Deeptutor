...
export function LearningBoard({
  ...
  modules: BoardModule[];
  revision: number;
  // Fork: `language`, not a `zh` boolean. Both things this component hands
  // the value to — `formatRelative` and `ObjectiveDetail` — already take a
  // `Language`, because a boolean cannot express the fork's third one.
  language: Language;
                ...
                >
                  ...
                  {card.next_review_at && (
                    <div className="mt-1.5 flex items-center gap-1 text-[11px] text-[var(--muted-foreground)]">
                      ...
                      {t("Review")}{" "}
                      {formatRelative(card.next_review_at, language)}
                    </div>
                  )}
                </button>
