...
const ROUTE_TARGETS = [
  ...
  {
    route: "/co-writer/[docId]",
    requestPath: "/co-writer/perf-budget",
    budgetKb: 515,
  },
  {
...
function assertBuildPresent() {
  if (!fs.existsSync(BUILD_MANIFEST_PATH)) {
    throw new Error(
      "Missing .next build output. Run `npm run build` before `npm run perf:check`.",
    );
  }
}
...
async function waitForServer(baseUrl, processState) {
  ...
  while (Date.now() < deadline) {
    if (processState.exited) {
      throw new Error(
        `Next server exited before readiness:\n${processState.output}`,
      );
    }
    try {
  ...
  throw new Error(
    `Timed out waiting for the production server:\n${processState.output}`,
  );
}
...
function scriptChunks(html, baseUrl) {
  ...
  for (const match of html.matchAll(
    ...
    if (relative.startsWith("static/") && relative.endsWith(".js"))
      chunks.add(relative);
  }
  return chunks;
}
...
async function loadRouteChunks(baseUrl, requestPath) {
  // `redirect: "manual"` is the whole point. `fetch` follows redirects by
  // default, and on any checkout that has accounts every route answers with a
  // redirect to `/login`. Following it hands each route the *login page's*
  // chunks, so every row becomes identical, the intersection that defines the
  // app shell swallows all of them, and each route measures 0KB — every budget
  // passes without a single byte being weighed. The shell is wrong too: it
  // gains `app/(auth)/login/page-*.js`, which is one page's weight, not the
  // shell's (measured: 423KB against a true 414KB).
  //
  // Silently measuring nothing is worse than failing, so a redirect is an
  // error. Run the measurement against a build with no accounts configured.
  ...
  if (!response.ok) {
    throw new Error(
      `${requestPath} returned HTTP ${response.status} during route measurement`,
    );
  }
...
function intersection(sets) {
  ...
  return sets
    ...
    .reduce(
      (shared, current) =>
        new Set([...shared].filter((item) => current.has(item))),
      new Set(sets[0]),
    );
}
...
async function main() {
  ...
  try {
    ...
    for (const target of ROUTE_TARGETS) {
      rows.push({
        ...target,
        chunks: await loadRouteChunks(server.baseUrl, target.requestPath),
      });
    }
    ...
    console.log(
      "Route budgets (raw production JS; framework and root app shell excluded):",
    );
    let failed = false;
    for (const row of rows) {
      const routeChunks = difference(
        row.chunks,
        frameworkChunks,
        appShellChunks,
      );
      failed =
        printRow(row.route, kb(sumChunkSizes(routeChunks)), row.budgetKb) ||
        failed;
    }
    failed =
      printRow(
        "root-app-shell",
        kb(sumChunkSizes(appShellChunks)),
        ROOT_SHELL_BUDGET_KB,
      ) || failed;
