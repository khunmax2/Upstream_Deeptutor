...
// A destination may carry an #anchor (v1.6.3 serves every settings screen
// from one page that way). The anchor is part of the destination but not
// part of the route, so resolve the page without it.
...
function routeExists(routePath: string): boolean {
  routePath = stripFragment(routePath);
...
test("every graph node path points at a real page", () => {
  const stale = (graph.nodes as { path: string }[])
    .map((n) => n.path)
...
test("every graph control is well-formed", () => {
  for (const node of graph.nodes as {
    id: string;
    path: string;
    controls: any[];
  }[]) {
    assert.ok(
      node.id && node.path.startsWith("/"),
      `node ${node.id}: bad id/path`,
    );
    assert.ok(
      Array.isArray(node.controls) && node.controls.length,
      `node ${node.id}: no controls`,
    );
    for (const control of node.controls) {
      assert.ok(
        control.capability,
        `node ${node.id}: control without capability id`,
      );
      assert.ok(
      ...
      assert.ok(
        Array.isArray(control.aliases) &&
          control.aliases.every((a: any) => typeof a === "string"),
