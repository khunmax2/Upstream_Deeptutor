...
# The languages a mastery prompt pack actually ships for. A language outside
# this set still loads — PromptManager falls back to English — so the fork
# appends its language directive instead of shipping a silently English prompt.
PACKAGED_PROMPT_LANGUAGES = frozenset({"en", "zh"})
...
def _load_system_prompt(language: str) -> str:
    """The mastery system prompt, in the reader's language where one exists.

    v1.6.6 replaced `prompts/<lang>/system.md` with a PromptManager pack
    (`mastery_loop.yaml`); that structure is upstream's and is used as it is.
    What it does not carry is the fork's third language — packs ship for `en`
    and `zh` only, and PromptManager answers a missing language with the English
    pack rather than an error, which is the right base but silently English for
    a Thai learner. So the language directive is appended whenever the language
    asked for is not one the packs cover, exactly as the `system.md` loader did.
    """
    ...
    loop_section = pack.get("loop") if isinstance(pack.get("loop"), dict) else {}
    text = str((loop_section.get("system") if loop_section else "") or "").strip()
    if lang in PACKAGED_PROMPT_LANGUAGES:
        return text
