...
def _system_text(language: str, name: str, budget: int, kind: str = "") -> str:
    ...
    if lang == "th":
        if is_partner:
            framing = (
                f"ในรอบนี้คุณเชื่อมต่อกับพาร์ทเนอร์ของผู้ใช้ «{name}» — ผู้ช่วยที่มีบุคลิก "
                f"ฐานความรู้ และทักษะเป็นของตัวเอง คุณสามารถปรึกษามันผ่านเครื่องมือ "
                f"`consult_subagent` เสมือนเพื่อนร่วมงานอิสระ (เช่น ใช้ฐานความรู้หรือมุมมอง"
                f"เฉพาะของมันมาช่วยตอบ) บทสนทนาระหว่างคุณกับมันจะถูกบันทึกเป็นเซสชันสมบูรณ์"
                f"ในประวัติของพาร์ทเนอร์นั้น และกระบวนการตอบของมันจะแสดงให้ผู้ใช้เห็นแบบเรียลไทม์"
            )
        else:
            framing = (
                f"ในรอบนี้คุณเชื่อมต่อกับเอเจนต์ภายนอกในเครื่องของผู้ใช้ «{name}» คุณสามารถ"
                f"ถามมันผ่านเครื่องมือ `consult_subagent` โดยมอบหมายงานที่มันทำได้ดีกว่าบนเครื่อง"
                f"ของผู้ใช้ — เช่น ตรวจสอบโค้ดเบส จำลองบั๊ก รันคำสั่ง "
                f"กระบวนการทำงานทั้งหมดของมันจะแสดงให้ผู้ใช้เห็นแบบเรียลไทม์"
            )
        return (
            f"{framing}\n\n"
            f"- ในรอบนี้คุณปรึกษามันได้มากที่สุด {budget} ครั้ง แต่ละผลลัพธ์จะบอกว่าเหลืออีกกี่ครั้ง "
            f"มันจะจำคำถามก่อนหน้าของคุณในรอบนี้ได้ คุณจึงถามเจาะลึกต่อได้\n"
            f"- เมื่อได้ข้อมูลเพียงพอแล้ว ให้หยุดเรียกเครื่องมือ แล้วตอบผู้ใช้ด้วยน้ำเสียงของคุณเอง "
            f"— อย่าสวมรอยเป็นมัน หรือถ่ายทอดคำพูดของมันด้วยมุมมองบุคคลที่หนึ่ง"
        )
    if is_partner:
        framing = (
