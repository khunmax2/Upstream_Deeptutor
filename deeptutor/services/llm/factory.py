...
async def _complete_with_resolved_config(
    ...
    *,
    ...
    Fork: returns the whole response rather than ``.content``. ``complete()``
    takes the content itself, one line below each call site, and
    ``complete_with_usage()`` — which needs ``.usage`` for token and cost
    accounting — becomes a thin wrapper instead of a second copy of this
    orchestration that has to be kept in step by hand.
    """
...
async def complete(
    ...
    **kwargs: Any,
    ...
    response = await _complete_with_resolved_config(
        config,
        provider_spec,
        prompt=prompt,
...
async def complete_with_usage(
    ...
    **kwargs: Any,
    ...
    """Same call as :func:`complete`, but returns the whole response so the
    caller can read ``.usage`` (prompt/completion/total tokens + cost), which
    ``complete()`` drops when it takes ``.content``.

    v1.6.6 moved the orchestration into ``_complete_with_resolved_config``; this
    now shares it rather than repeating it, so the two can no longer drift.
    """
    caller_extra_headers = kwargs.pop("extra_headers", None)
    reasoning_effort = kwargs.pop("reasoning_effort", None)
    image_data = kwargs.pop("image_data", None)
