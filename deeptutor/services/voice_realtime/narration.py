...
FILL_NO_OPTION_LINE = "ช่องนั้นไม่มีตัวเลือกตามที่พูดครับ ลองพูดชื่อตัวเลือกอีกครั้งครับ"
# Edit command with no field named and no fill done yet this call.
EDIT_NO_FIELD_LINE = "ยังไม่รู้ว่าช่องไหนครับ บอกชื่อช่องด้วยครับ เช่น ล้างช่องค้นหา"
...
def ambiguous_click_line(candidates: list[str]) -> str:
    """Ask-back for a click tie that NAMES the tied buttons.

    "หมายถึง X หรือ Y ครับ" gives the caller something to answer (and tells
    the maintainer why the resolver tied); the generic "พูดชื่อเต็ม" is only
    the fallback when no tie members are known. Capped at three names — a
    spoken list longer than that is noise.
    """
    names = [c for c in candidates if c][:3]
    if not names:
        return CLICK_AMBIGUOUS_LINE
    return f"หมายถึง {' หรือ '.join(names)} ครับ"
