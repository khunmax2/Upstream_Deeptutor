...
def parse_language(language: Any) -> str:
    ...
    Resolves the spelled-out aliases of the locales that ship prompt
    resources ("english"/"en", "chinese"/"cn"/"zh", "thai"/"th") and otherwise
    passes the code through normalized, so a request for Japanese stays ``"ja"``
    instead of collapsing to Chinese (#712). Callers that load per-language
    prompt files fall back to English when a code has no files of its own; what
    makes the model actually answer in that language is the directive built from
    this code, not the prompt file.

    Args:
        language: Language configuration value ("zh"/"en"/"th"/"Chinese"/"ja"/…)
