...
async def _youtube_transcript(
    ...
    def fetch() -> tuple[list[dict[str, Any]], str]:
        ...
        # Fork. An explicit ``language`` from the request still wins; otherwise
        # the clip is read in the language it was spoken in, by the same rule
        # the reading workspace uses (deeptutor/reading/transcript_language.py)
        # -- upstream's fixed ``zh-CN, zh-Hans, zh, en`` showed a Thai learner
        # Chinese captions on an English keynote.
        if not language and hasattr(api, "list"):
            chosen = choose_track(
                [
                    CaptionTrack(
            ...
            if chosen is None:
                return [], ""
            return normalize_cues(list(chosen.source.fetch())), chosen.language_code
...
def _caption_choice(rows: Any, language: str) -> dict[str, Any] | None:
    ...
    if not captions:
        return None
    # Fork. Same rule as the youtube_transcript_api path: an explicit language
    # wins, otherwise the spoken language first (transcript_language.py).
    tracks = [
        CaptionTrack(
            language_code=str(row.get("languageCode") or row.get("language_code") or ""),
            generated=bool(row.get("autoGenerated") or row.get("auto_generated")),
            source=row,
        )
        for row in captions
    ]
    chosen = choose_track(
        tracks,
        interface_language=interface_language_code(),
        order=[language, *DEFAULT_ORDER] if language else None,
    )
