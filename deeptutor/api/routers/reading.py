...
async def upload_material(
    ...
    try:
        ...
        if Path(filename).suffix.lower() in _MEDIA_EXTENSIONS:
            ...
            # Fork: the learner-upload ledger is written here, before the work is
            # handed to the background task. Upstream's restructure returns from
            # inside this branch, so recording it after the return — the shape
            # the old code had — would silently stop tracking media uploads.
