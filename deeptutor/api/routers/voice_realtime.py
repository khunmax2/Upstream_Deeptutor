...
Client → server:
  ...
  * ``{"type": "ui_inventory", "inventory": [{"i", "tag", "label",
    ...
    interactive elements (observe transport for the in-page agent loop,
    PLAN_inpage_agent_parity Phase B; currently unrequested)
...
Server → client (all JSON except the audio payload frames):
  ...
  * ``{"type": "ui_scan"}``  request the indexed element inventory (agent observe)
  * ``{"type": "error", "message": …}``
