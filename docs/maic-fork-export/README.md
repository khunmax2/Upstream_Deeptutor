...
## What is still not here — and where it went

`deeptutor/services/config/openmaic_bridge.py`, the gatekeeper
(`deploy/openmaic-gatekeeper/`) and the compose overlay
(`deploy/docker-compose.openmaic.yml`) were left unexported because they were
live code in the repository, reachable through `deploy/OPENMAIC_RUNBOOK.md` and
`deploy/OPENMAIC_EMBED.md`.

**That is no longer true of `main`.** On 2026-09-09 the integration was removed
from `main` so it could be designed again from a clean import, and those five
files went with it. They are not lost — `main` at that moment is kept whole on

    archive/main-2026-09-09      (5c6ed4295)

Read them there:

```bash
git show archive/main-2026-09-09:deploy/OPENMAIC_RUNBOOK.md
git show archive/main-2026-09-09:deeptutor/services/config/openmaic_bridge.py
...
would drift from the one in use.

One consequence to expect: `export_upstream_patches.py` and
`check_openmaic_contract.py` both need an `integration/maic` checkout to say
anything. Until the next import exists they will fail, and that is the correct
