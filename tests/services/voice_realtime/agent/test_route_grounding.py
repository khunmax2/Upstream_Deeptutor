...
class TestLandedPath:
    def test_full_href_reduces_to_pathname(self):
        assert landed_path("http://localhost:3783/knowledge-bases") == "/knowledge-bases"

    def test_query_is_stripped_but_the_anchor_is_kept(self):
        # Since upstream v1.6.3 the anchor *is* the settings destination, so it
        # survives where the query string does not.
        assert landed_path("http://x/settings?tab=web#search") == "/settings#search"

    def test_trailing_slash_trimmed(self):
        assert landed_path("http://x/knowledge-bases/") == "/knowledge-bases"
