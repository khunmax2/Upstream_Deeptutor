...
<<<<<<< Updated upstream
=======
## Everything this fork changed inside the OpenMAIC subtree, written down — 2026-09-09

`docs/maic-fork-export/` exists because the integration may be rebuilt from a
clean import, and a diff does not survive that: it says what changed and not why,
and the commits that carry the reasoning become unreachable the moment the
subtree is re-imported.

Reference point is `8475f82c6`, the squashed import of upstream `fb0a9f22`.
Everything after it that touches `integration/maic` is ours — 22 commits, 127
files, +4,557 / −2,990 — grouped into six areas with what each was for, what it
cost, and what went wrong while making it.

`why.md` is the whole point: every commit message in order. Those were written
long deliberately, and they hold decisions the code cannot express — why avatars
are prefixed at the render site rather than at the constants (`as const` widening,
and a persisted profile avatar), why `asset()` has to be idempotent (two layers
apply it), why a literal grep misses labels built from ternaries.

`patches/` holds one patch per commit in apply order, so a rebuild can take them
one at a time instead of all or nothing. `pnpm-lock.yaml` and binary assets are
excluded — regenerable, and unreadable as guidance. `full.patch` is deliberately
absent, being the sum of the others.

Also records what a rebuild should expect to fight: the lockfile conflicts on
every upstream dependency change, the twelve locale files move together, eight
files were edited by more than thirty lines and need reading rather than
